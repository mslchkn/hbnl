        ...
        <div class="col-lg-12">
            <form class="row login_form" id="metaForm">
                @foreach ($plugins as $plugin)
                    ...
                    @foreach ($plugin->getFields() as $field)
                        <div class="@if ($field['type'] != 'color') col-md-12 form-group @else col-md-1 @endif">
                            <input type="{{$field['type']}}" class="form-control" id="top-banner-image" name="{{$field['name']}}"
                                   placeholder="{{$field['label']}}" onfocus="this.placeholder = ''">
                        </div>
                    @endforeach
                @endforeach
